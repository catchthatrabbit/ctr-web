...
const generateAllApiInstances = ():Record<"instances",Record<string,AxiosInstance>> => {
    return {
        instances:{
...
export const getAllStats = () => {
    ...
    return Object.keys(allApi.instances).map(key => allApi.instances[key].getInstance().get("/stats")) as Array<Promise<{data:STATS_RESPONSE}>>
}

export const getAllStatsCharts = () => {
    const allApi = generateAllApiInstances();
    return Object.keys(allApi.instances).map(key => allApi.instances[key].getInstance().get("/stats/chart")) as Array<Promise<{data:STATS_CHARTS_RESPONSE}>>
}


export const getAllRegionsMaturedBlocks = (limit = 5, offset = 0) => {
    const allApi = generateAllApiInstances();
    return Object.keys(allApi.instances).filter(key => (key !== POOL_NAME_ENUM.US && key !== POOL_NAME_ENUM.US_BACKUP))
    .map(key => allApi.instances[key].getInstance().get(`/matured_blocks?limit=${limit}&offset=${offset}`)) as Array<Promise<{data:MATURED_RESPONSE}>>
}