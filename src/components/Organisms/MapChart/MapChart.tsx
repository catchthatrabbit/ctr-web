...
const horClassName = "xl-flex-col--2 lg-flex-col--2 md-flex-col--6 sm-flex-col--12 xs-flex-col--12";
...
const MapChart = ({ children, infoItems, isLoading }:IMapChart) => {
    ...
    const ConfiguredInfoBox = () => <>
        <InfoBox boardClassNameHor={horClassName} isLoading={isLoading} items={infoItems}
        loadingComponent={<InfoBoxLoadingSkeleton boardClassNameHor={horClassName}
        loadingPlaceholder={<LoadingPlaceholder />} />} />
    </>;  

    return (
      <div className={styles.mapRoot}> 
          { (!mobile && !tablet) && <><Spacer variant="lg" /><ConfiguredInfoBox /><Spacer variant="lg" /></>}
          <div>
            {children}
          </div>
          { (mobile || tablet) && <><Spacer variant="lg" /><ConfiguredInfoBox /></>}
      </div>
    )
}
