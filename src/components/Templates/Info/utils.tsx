...
const calculateProfitability = async (hashrate: number) => {
...
export const convertWalletInfoResponse2ComputingInformation = (
  ...
  const formatProfitabilityText = async (value: number) => {
    ...
    try {
      const profitability = await calculateProfitability(value);
      if (profitability) {
        const formatted = TextFormat.getProfitabilityText(profitability.revenue.toFixed(2), null, profitability.period, true);
        return `${formatted.prefix}${formatted.text}${formatted.suffix}`;
      }
      return "Calculating…";
    } catch (error) {
