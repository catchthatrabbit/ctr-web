...
import { aggregateNumbers, convertPoolChartDataToChartData, convertPoolChartDataToRadialInfoBox, reduceList } from "../utils";
...
const useRadialBarChartData = () => {
    ...
    const allPoolChartsData = statsChartsData?.reduce<{ allPoolChartsData: Array<{poolCharts:Array<{x:number, y:number}>}>,
    ...
    if(Array.isArray(allPoolChartsData?.allLastBlockFound) && allPoolChartsData.allLastBlockFound.length > 0)
